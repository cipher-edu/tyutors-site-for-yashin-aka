...
# --- Inline Classlar (O'zgarishsiz qoladi) ---
class CourseInline(admin.TabularInline):
    model = Course
    fields = ('title', 'order', 'video_url', )
    extra = 1; ordering = ('order',); show_change_link = True
    verbose_name = "Modul kursi"; verbose_name_plural = "Modul kurslari"

class TestInline(admin.StackedInline):
    model = Test
    fields = ('title', 'description', 'passing_score_percent')
    can_delete = False; verbose_name = "Modul testi"; verbose_name_plural = "Modul Testi"
...
class QuestionInline(admin.StackedInline):
    ...
    verbose_name = "Test savoli"; verbose_name_plural = "Test savollari"


# --- Model Admin Classlari ---
...
class ModuleAdmin(admin.ModelAdmin):
    ...
    ordering = ('order', 'title') # Bu yerda to'g'ri, chunki Module'da order bor
    fieldsets = ( (None, {'fields': ('title', 'order', 'description')}),
                  ('Vaqt Belgilari', {'fields': ('created_at', 'updated_at'), 'classes': ('collapse',)}) )
    ...
    @admin.display(description='Kurslar soni', ordering='course_count_annotation')
    def display_course_count(self, obj): return obj.course_count_annotation
    @admin.display(description='Test mavjud', boolean=True)
    def display_has_test(self, obj): return hasattr(obj, 'test') and obj.test is not None
    @admin.display(description='Yaratilgan sana', ordering='created_at')
...
class CourseAdmin(admin.ModelAdmin):
    ...
    ordering = ('module__order', 'order', 'title') # Kursda ham order bor, modulniki bilan birga ishlatamiz
    fields = ('module', 'title', 'order', 'content', 'video_url')
    list_select_related = ('module',)
    formfield_overrides = { models.TextField: {'widget': admin.widgets.AdminTextareaWidget(attrs={'rows': 15, 'cols': 80})} }
    @admin.display(description='Modul', ordering='module__title')
    ...
    @admin.display(description='Video mavjud', boolean=True)
    def has_video(self, obj): return bool(obj.video_url)
    created_at_formatted = ModuleAdmin.created_at_formatted
...
class TestAdmin(admin.ModelAdmin):
    ...
    ordering = ('module__order', 'title') # Test'da 'order' yo'q, modul'niki bo'yicha tartiblaymiz
    fields = ('module', 'title', 'description', 'passing_score_percent')
    inlines = [QuestionInline]; list_select_related = ('module',)
    module_link = CourseAdmin.module_link
    def get_queryset(self, request):
        queryset = super().get_queryset(request)
        return queryset.annotate(question_count_annotation=Count('questions'))
    @admin.display(description='Savollar soni', ordering='question_count_annotation')
...
class QuestionAdmin(admin.ModelAdmin):
    ...
    # --- MUAMMONI HAL QILISH: `ordering` QATORINI OLIB TASHLAYMIZ ---
    # ordering = ('test', 'order') # Bu qator olib tashlandi! Django modeldagi Meta.ordering'dan foydalanadi.
    # Agar alohida tartib kerak bo'lsa:
    # ordering = ('test__module__order', 'test__title', 'order')
    fields = ('test', 'text', 'order')
    inlines = [AnswerInline]
    list_select_related = ('test', 'test__module')
    @admin.display(description='Savol matni (qisqa)', ordering='text')
    def short_text(self, obj): return mark_safe(f"{obj.text[:70]}...") if len(obj.text) > 70 else obj.text
    @admin.display(description='Test', ordering='test__title')
    ...
    @admin.display(description='Javoblar soni', ordering='answer_count_annotation')
    def display_answer_count(self, obj): return obj.answer_count_annotation


# --- Foydalanuvchi bilan bog'liq modellar (O'zgarishsiz qoladi) ---
...
class UserCourseProgressAdmin(admin.ModelAdmin):
    ...
    @admin.display(description='Kurs', ordering='course__title')
    def course_link(self, obj):
        link = reverse("admin:core_course_change", args=[obj.course.id])
        return format_html('<a href="{}">{}</a>', link, obj.course.title)
    @admin.display(description='Modul', ordering='course__module__title')
    def module_title(self, obj): return obj.course.module.title
    @admin.display(description='Tugatilgan sana', ordering='completed_at')
...
class UserTestResultAdmin(admin.ModelAdmin):
    ...
    @admin.display(description='Modul', ordering='test__module__title')
    def module_title(self, obj): return obj.test.module.title
    @admin.display(description='O\'tganligi', boolean=True, ordering='passed')
    def passed_status(self, obj): return obj.passed
    @admin.display(description='Urinish vaqti', ordering='attempted_at')
